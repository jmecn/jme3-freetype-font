...
import io.github.jmecn.math.*;
import io.github.jmecn.shape.LineTo;
import io.github.jmecn.shape.MoveTo;
...
public class PrismTextLayout implements TextLayout {
    ...
    private Font font;        /* Single font text (null for rich text) */
    ...
    @Override
    public boolean setContent(String text, Font font) {
        reset();
        this.spans = null;
        this.font = font;
        this.strike = font.getStrike(IDENTITY);
    ...
    public BaseBounds getBounds(TextSpan filter, BaseBounds bounds) {
        ...
        if (filter != null) {
            for (int i = 0; i < lines.length; i++) {
                ...
                for (int j = 0; j < lineRuns.length; j++) {
                    ...
                    Vector2f location = run.getLocation();
        ...
        return bounds.deriveWithNewBounds(left, top, right, bottom);
    }
            ...
            if (text == null || spans == null) {
                ...
                for (TextRun r: runs) {
                    ...
                    if (r.getTextSpan() != null && r.getTextSpan().getText().equals(text)) {
                        BaseBounds textBounds = new RectBounds();
...
//        return outline;
            ...
            if (layoutCache != null) {
                if (copyCache()) {
                    ...
                    if (layoutCache.lines != null) {
                        ...
                        logicalBounds = logicalBounds.deriveWithNewBounds(0, ascent,
                                layoutWidth, layoutHeight + ascent);
                    }
                }
            }
        }
    ...
    private void layout() {
        ...
        float ascent = lines[0].getBounds().getMinY();
        layoutHeight = lineY;
        logicalBounds = logicalBounds.deriveWithNewBounds(0, ascent, layoutWidth,
                layoutHeight + ascent);
